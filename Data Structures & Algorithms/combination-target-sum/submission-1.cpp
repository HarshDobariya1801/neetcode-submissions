...
private:
    ...
    // }
    void solve(int i, int n, vector<int> &nums,int target,vector<vector<int>> &ans,vector<int> &temp){
        ...
        if(nums[i] <= target){
            temp.push_back(nums[i]);
            solve(i,n,nums,target-nums[i],ans,temp);
            temp.pop_back();
        }

        solve(i+1,n,nums,target,ans,temp);
...
public:
    vector<vector<int>> combinationSum(vector<int>& nums, int target) {
        ...
        solve(0,n,nums,target,ans,temp);
